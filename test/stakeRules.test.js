...
test('the ladder is exactly the agreed set', () => {
  assert.deepEqual(STAKE_TIERS, [0.50, 2]);
  assert.deepEqual(ALL_STAKES, [0, 0.50, 2]);
  assert.equal(MIN_STAKE, 0.50);
  assert.equal(MAX_STAKE, 2);
});
...
test('an amount between rungs is refused', () => {
  ...
  for (const v of [0.10, 0.25, 0.26, 0.75, 1, 1.5, 3, 5, 37.42, 100, 250])
...
test('the refusal names the ladder rather than just saying no', () => {
  ...
  assert.ok(msg.includes('$0.50') && msg.includes('$2'), msg);
...
test('float noise does not knock a stake off its rung', () => {
  // 0.1 + 0.4 is 0.5000000000000001. Compared as a float that is not 0.50
  // and the player lands in a room of their own.
  assert.equal(isStake(0.1 + 0.4), true, '0.50 the hard way');
  assert.equal(isStake(1.9 + 0.1), true, '2 the hard way');
  assert.equal(stakeRangeError(0.1 + 0.4), null);
});

test('a payment buys the largest tier it covers', () => {
  assert.equal(tierFor(0.50), 0.50);
  assert.equal(tierFor(2), 2);
  assert.equal(tierFor(1), 0.50, 'a dollar is no longer a rung, so it buys the $0.50 room');
});
...
test('a small overpay still buys the tier, it does not fail', () => {
  ...
  assert.equal(tierFor(2.04), 2);
  assert.equal(tierFor(0.99), 0.5, 'an underpay drops to the tier it does cover');
  assert.equal(tierFor(150), 2, 'and a large overpay caps at the top rung');
});
